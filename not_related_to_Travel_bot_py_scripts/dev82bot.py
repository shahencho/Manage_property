#get_selection_of_building_from_db_by_teleg_id# #print("Hello, World!")
# # 7072468689:AAE5wCELSvL7T6V81S7-CXrzLwk2pvwAxDE
...
#     # Handle all messages with the hello function
...
from Find_Dept_By_Code import Realize_worksheet_by_building
...
# add_new_user_to_db(449708378, "shah", "kuku")
...
#Carav agbyut 5946410507:AAF5VAgSyF3DirCVWuLVEF9eqLt8yNKrqhg
#devbot 7256193277:AAEMtDw3bqG8DgImbcR0Y4w8Znq39NatknA

token = '7256193277:AAEMtDw3bqG8DgImbcR0Y4w8Znq39NatknA'
bot = telebot.TeleBot(token)
# HELP = """

HELP = "This is the help message for your bot."

if if_user_is_new(5215699499,bot.get_me().id ):
    add_new_user_to_db(5215699499, "Xuan", "Fran" , bot.get_me().id)


# Define function with message handler
...
# print(resp)
# print(resp+"this is the end !!!!!!!!!!!")
bot_info = bot.get_me().id
...
def Find_Code_By_address(message_chat_id,kotorak, bnak_hamar, worksheet_sequence):
    ...
    temp21  = update_bill_code_for_given_user_to_db(message_chat_id, final_duty_to_be_paid, bot.get_me().id) # 0 > rename to Depnow

    print ("value of fucntion  - > update_bill_code_for_given_user_to_db(message_chat_id, final_duty_to_be_paid bot.get_me().id )  = " +  temp21 )

    temp67 = update_selection_of_F_N_P_to_db(message_chat_id, "bill_code" , combined_row, bot.get_me().id)

    print ("""update_selection_of_F_N_P_to_db(message_chat_id, 'bill_code' , combined_row bot.get_me().id )  = """ + temp67) #-> rename to  update_db_col_id_value
...
def start(message):
    # global current_user_id_start
    current_user_id_start = 99898
    # global Cell_To_Find
    current_user_id_start = message.chat.id
    ...
    # print("building_you_selected suring start  = " + str(building_you_selected))

    if if_user_is_new(current_user_id_start, bot.get_me().id):
        add_new_user_to_db(current_user_id_start, message.chat.first_name, message.chat.last_name, bot.get_me().id )

#

        markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
        btn1 = types.KeyboardButton("Hamatirutyun1")
        btn2 = types.KeyboardButton("Hamatirutyun -> Նորավանի ")
        ...
        bot.send_message(message.chat.id,
                         text="Ողջույն, " + first_name_of_current_user + " : Ես Innova բոտն եմ  please choose what hamatirutyun are you interested? ", reply_markup=markup)
...
#
        ...
        last_code_saved_in_db = get_selection_of_building_from_db_by_teleg_id(current_user_id_start, "bill_code", bot.get_me().id)
        print ("get_selection_of_building_from_db_by_teleg_id(current_user_id_start) =   " + str(last_code_saved_in_db) )
        ...
        text_to_send_registered_user = text_to_send_registered_user.format(first_name_of_current_user,
                                                                           get_selection_of_building_from_db_by_teleg_id(current_user_id_start, "bill_code",bot.get_me().id))
        ...
        text_to_print = str(
            ...
            last_code_saved_in_db) + " կոդով սեփականության պարտքը, կամ սեղմեք «Վերադարձ գլխավոր մենյու», եթե ցանկանում եք այլ որոնում իրականցել"
...
def func(message):
    ...
    message_chat_id = message.chat.id
    current_user_id_text = message.chat.id

    print("building_you_selected = " + str(building_you_selected))

    if if_user_is_new(message_chat_id, bot.get_me().id):
        add_new_user_to_db(message_chat_id, message.chat.first_name, message.chat.last_name,bot.get_me().id )
    ...
    current_val_of_building_from_db_by_teleg_id  = get_selection_of_building_from_db_by_teleg_id (current_user_id_text,  "building", bot.get_me().id)
    print(str(current_val_of_building_from_db_by_teleg_id) + "get_selection_of_building_from_db_by_teleg_id (current_user_id_text,  building) ")

    if (message.text == "Hamatirutyun1"):
        print("WE ARE IN Hamatirutyun -> ՆորավանիHamatirutyun -> ՆորավանիHamatirutyun -> ՆորավանիHamatirutyun -> ՆորավանիHamatirutyun -> ՆորավանիHamatirutyun -> ՆորավանիHamatirutyun -> ՆորավանիHamatirutyun -> Նորավանի")
        ...
        bot.send_message(message.chat.id,
                         text="Ողջույն, " + message.chat.first_name + " : Ես Նորավանի բոտն եմ և կօգնեմ Ձեզ տեղեկություն ստանալ Ձեր սեփականության ընթացիկ պարտքի կամ կանխավճարի մասին։ Հուսով եմ պարտք չէ, կանխավճար է :) \nԱյս պահին հասանելի է " + str(current_month_am) + " ամիսը:\n", reply_markup=markup)
        ...
        update_selection_of_building_to_db(current_user_id_text, building_you_selected , bot.get_me().id) #save this selection to DB in column 'building'
        ...
        update_selection_of_building_to_db(current_user_id_text, building_you_selected , bot.get_me().id) #save  building_you_selected selection to DB  > it will save in  column "col1"
    ...
    # ====Avan6 worksheet 2 -------------------------------------------------
        ...
        temp_val = Find_Dept_By_Code(x, current_user_id_text)
    ...
    # print(type(x))
    elif (x.isnumeric() == True) and get_selection_of_building_from_db_by_teleg_id(  current_user_id_text, "f_n_p", bot.get_me().id) == "flat":
        print("numeric")


        building_you_selected_v2 = get_selection_of_building_from_db_by_teleg_id(message_chat_id, "building", bot.get_me().id)
        print('get_selection_of_building_from_db_by_teleg_id(message_chat_id, "building") = ', building_you_selected_v2  )
        curr_wsheet = Realize_worksheet_by_building(building_you_selected_v2)
        print('curr_wsheet = Realize_worksheet_by_building(building_you_selected_v2)' + str(curr_wsheet)  )
        ...
        if int(x) < 281 and (
            ...
            if curr_wsheet == 0:
                ...
                if (int(get_kototak_from_selection) == 17 or int(get_kototak_from_selection) == 20):
                    dutytopay = Find_Code_By_address(message_chat_id, get_kototak_from_selection, x, curr_wsheet)
...
"""
                    ...
                    current_billcode_in_db = get_selection_of_building_from_db_by_teleg_id(current_user_id_text, "bill_code", bot.get_me().id)
                    print ("current_billcode_in_db = " + str(current_billcode_in_db))

                    bot.send_message(message.chat.id, text = current_billcode_in_db)
    ...
    # building_you_selected_v2 = get_selection_of_building_from_db_by_teleg_id(message_chat_id, "col1")
    ...
    elif (x.isnumeric() == True) and get_selection_of_building_from_db_by_teleg_id(current_user_id_text, "f_n_p", bot.get_me().id) == "nkugh" and current_val_of_building_from_db_by_teleg_id != "uknown" or ( #  building_you_selected != "uknown"  - > this must be fixed to understand when this is happpeneing..
            x.isnumeric() == True) and get_selection_of_building_from_db_by_teleg_id(  current_user_id_text, "f_n_p", bot.get_me().id)== "parking" and current_val_of_building_from_db_by_teleg_id != "uknown":
        ...
        get_current_f_n_p  = get_selection_of_building_from_db_by_teleg_id(current_user_id_text, "f_n_p", bot.get_me().id)
        ...
        dutytopay = Find_Dept_By_Code(combined_code, current_user_id_text)
        ...
        if dutytopay == "myundefined":
            ...
            update_bill_code_for_given_user_to_db(current_user_id_text, dutytopay, bot.get_me().id)
            # update_new_user_to_db(current_user_id, building_you_selected_v2[6:])
            # print("settings.Cell_To_Find " + strupdate_selection_of_F_N_P_to_db(building_you_selected_v2[6:]))
            ...
            if (get_selection_of_building_from_db_by_teleg_id(  current_user_id_text, "f_n_p", bot.get_me().id) == "nkugh"):
                tmp_text1 = "նկուղի"
            elif get_selection_of_building_from_db_by_teleg_id(  current_user_id_text, "f_n_p", bot.get_me().id) == "parking":
                tmp_text1 = "կայանատեղիի"


            get_selection_of_building_from_db_by_teleg_id(  current_user_id_text, "f_n_p", bot.get_me().id)
...
"""
    ...
    #     print("we are in section of parking - this block doesn't really required." )
        ...
        last_bill_code_saved_in_db = get_selection_of_building_from_db_by_teleg_id(current_user_id_text, "bill_code", bot.get_me().id)

        leng = len(last_bill_code_saved_in_db)

        if last_bill_code_saved_in_db[5:6] == "-":  # A-553-2
            print("this is 55/3 shenq")
            get_kotorak = last_bill_code_saved_in_db[4:5]  # get_kotorak = '3' ; ->>> print("this is 55/3 shenq")
            get_bnak_hamar = last_bill_code_saved_in_db[6:leng]  # for case with 'A-5517-10 - > 10

        else:
            get_kotorak = last_bill_code_saved_in_db[4:6]  # for case with 'A-5517-10 - > 17

            get_bnak_hamar = last_bill_code_saved_in_db[7:leng]  # for case with 'A-5517-10 - > 10
        ...
        get_firstletter_of_code = last_bill_code_saved_in_db[0:1]
        ...
        if get_firstletter_of_code == "A":
            ...
            dept_for_this_user_in_google_doc = Find_Dept_By_Code(last_bill_code_saved_in_db, current_user_id_text)
        ...
        if dept_for_this_user_in_google_doc != "99999999":
            ...
            text_to_print = "\nԱյս պահին հասանելի է " + str(current_month_am) + " ամիսը:\nՀարգելի սեփականատեր,{} դրությամբ ձեր բալանսը հետևյալ " + str(last_bill_code_saved_in_db) +" կոդի համար  կազմում է՝"
            temp_text = "uknownnnn"
...
"""
                ...
                bot.send_message(message.chat.id, text = str(last_bill_code_saved_in_db))
...
# ===============================================================
    ...
    # select bnakaran part_---------------------------------------bnakaranbnakaranbnakaranbnakaranbnakaranbnakaranbnakaranbnakaranbnakaranbnakaran
        ...
        building_you_selected_v2 = get_selection_of_building_from_db_by_teleg_id(message_chat_id, "building", bot.get_me().id)
        print('elif (message.text == "Բնակարան"    get_selection_of_building_from_db_by_teleg_id(message_chat_id, "building", bot.get_me().id) = ', building_you_selected_v2  )
        ...
        update_selection_of_F_N_P_to_db(current_user_id_text, "f_n_p", "flat", bot.get_me().id)
    ...
    # select Nkugh part_ՆկուղՆկուղՆկուղՆկուղՆկուղՆկուղՆկուղՆկուղՆկուղՆկուղՆկուղՆկուղՆկուղՆկուղՆկուղՆկուղՆկուղՆկուղՆկուղՆկուղՆկուղՆկուղՆկուղՆկուղՆկուղՆկուղՆկուղՆկուղՆկուղՆկուղՆկուղՆկուղՆկուղ
        ...
        building_you_selected_v2 = get_selection_of_building_from_db_by_teleg_id(message_chat_id, "building", bot.get_me().id)
        print('elif (message.text == "Նկուղ"    get_selection_of_building_from_db_by_teleg_id(message_chat_id, "building", bot.get_me().id) = ', building_you_selected_v2  )
        ...
        update_selection_of_F_N_P_to_db(current_user_id_text, "f_n_p", "nkugh", bot.get_me().id)
        ...
        update_selection_of_F_N_P_to_db(current_user_id_text, "f_n_p", "parking", bot.get_me().id)


        print( " in message.text ==  Բնակարան i just called  update_selection_of_F_N_P_to_db(teleg_id, f_n_p,  parking ) ")

        tempppp  = get_selection_of_building_from_db_by_teleg_id(message_chat_id, "f_n_p", bot.get_me().id)

        print ("get_selection_of_building_from_db_by_teleg_id(message_chat_id, 'f_n_p'), bot.get_me().id" +  str(tempppp))
...
#             time.sleep(20)  # Wait 20 seconds before restarting the bot
...
#option 2 , send every 5 minutes message to bot

def send_periodic_message():
    while True:
        try:
            # Send a message to the bot to simulate activity
            bot.send_message('CHAT_ID_HERE', 'Activity check message')
            logger.info("Sent periodic message to keep bot active")
            # Wait for 5 minutes (300 seconds) before sending the next message
            time.sleep(300)
        except Exception as e:
            logger.error(f"Error while sending periodic message: {e}")
...
if __name__ == '__main__':
    # Start a separate thread for sending periodic messages
    import threading
